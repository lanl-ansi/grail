...
if(par.out.plotmarketflowlims==1)
    ...
    tv=kron(out.int_qbar(:,out.guniqueind)',[1 1])';
    ...
    if(~isempty(par.tr.dmax_pos))
        ...
        tv=kron(out.int_dmax',[1 1])';
        plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), hold on,
        tv=kron(out.int_dmin',[1 1])';
        plot(tp,tv,'--','LineWidth',3), hold off, legend(num2str(par.tr.dmax_pos),'Location','SouthEast')
    ...
    if(~isempty(par.tr.smax_pos))
        ...
        tv=kron(out.int_smax',[1 1])';
        plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), hold on,
        tv=kron(out.int_smin',[1 1])';
        plot(tp,tv,'--','LineWidth',3), hold off, legend(num2str(par.tr.smax_pos),'Location','SouthEast')
...
if(par.out.plotsolflow==1)
    ...
    subaxis(1,3,1,'MarginLeft',0.05,'SpacingHoriz',0.05), 
    tv=kron([-out.supp_flow_int out.dgflows_int]',[1 1])';
    ...
    subaxis(1,3,3,'MarginRight',0.05,'SpacingHoriz',0.05),
    tv=kron([out.supp_flow_int out.gssol_int]',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
...
if(par.out.plotsolprice==1)
    ...
    tv=kron(out.trlmpnodal_int',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    if(par.out.units==1), title('All Nodal LMPs ($/mscf)','fontweight','bold'), else
    title('All Nodal LMPs (10$/kg)','FontWeight','bold'), end
    subaxis(1,2,2,'MarginRight',0.05,'SpacingHoriz',0.05),
    tv=kron(out.dglmp_int',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
...
if(par.out.plotopt==1)
    ...
    subplot(2,1,1), plot(out.tt0,out.ppopt,'LineWidth',3), axis('tight'), xlabel('time (hours)'), ylabel(''), 
    ...
    subplot(2,1,2), plot(out.tt0,out.qqopt,'LineWidth',3), axis('tight'), xlabel('time (hours)'), ylabel(''), 
...
if(par.out.plotsim==1)
    ...
    subplot(2,1,1), plot(out.tt,out.ppsim), axis('tight'), xlabel('time (hours)'), ylabel(''), 
    ...
    subplot(2,1,2), plot(out.tt,out.qqsim), axis('tight'), xlabel('time (hours)'), ylabel(''), 
...
if(par.out.plotcompabs==1)
    ...
    subplot(2,1,1), plot(out.ttcom,pdp), axis('tight'), xlabel('time (hours)'), ylabel(''), 
    ...
    subplot(2,1,2), plot(out.ttcom,pdq), axis('tight'), xlabel('time (hours)'), ylabel(''), 
...
if(par.out.plotcomprel==1)
    ...
    subplot(2,1,1), plot(out.ttcom,pdp*100), axis('tight'), xlabel('time (hours)'), ylabel(''), 
    ...
    subplot(2,1,2), plot(out.ttcom,pdq*100), axis('tight'), xlabel('time (hours)'), ylabel(''), 
...
%compression ratios, discharge pressure setpoints, compressor power
    ...
    subaxis(1,3,3,'MarginRight',0.05,'SpacingHoriz',0.05), 
    tv=kron(out.cpowopt_int'/1000,[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), 
    xlabel('hours'), ylabel(''), legend(num2str([1:out.n0.nc]')), 
...
%compression ratios, discharge pressure setpoints, compressor power
    ...
    subaxis(1,3,1,'MarginLeft',0.05,'SpacingHoriz',0.05), 
    plot(out.tt0,out.ppoutopt-out.ppinopt,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    ...
    subaxis(1,3,2,'SpacingHoriz',0.05), 
    plot(out.tt0,out.qqoutopt-out.qqinopt,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    legend(num2str([1:out.n0.ne]'),'Location','SouthEast'), 
    if(par.out.units==1), title('Flow Differentials (mmscfd)','FontWeight','bold'), else
        title('Flow Differentials (kg/s)','FontWeight','bold'), end
    subaxis(1,3,3,'MarginRight',0.05,'SpacingHoriz',0.05),     
    tv=kron((out.lmpout_int-out.lmpin_int)',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), 
    xlabel('hours'), ylabel(''), legend(num2str([1:out.n0.ne]')), 
...
if(par.out.plotcompshadow==1)
    ...
    subaxis(1,2,1,'MarginLeft',0.05,'SpacingHoriz',0.05), 
    tv=kron((out.mult0_pmax_int.*(abs(out.mult0_pmax_int)>10^(-3)))',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    legend(num2str([1:out.n0.nc]'),'Location','SouthEast'), 
    ...
    subaxis(1,2,2,'MarginRight',0.05,'SpacingHoriz',0.05),
    tv=kron((out.mult0_cmax_int.*(abs(out.mult0_cmax_int)>10^(-3)))',[1 1])';
    plot(tp,tv,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
...
if(par.out.plotaccuracy==1)
    ...
    subaxis(1,2,1,'MarginLeft',0.05,'SpacingHoriz',0.05), 
    plot(out.flowbalrel'*100,'LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    ...
    plot(out.flowbal','LineWidth',3), axis('tight'), xlabel('hours'), ylabel(''), 
    legend(num2str([1:out.n0.nv]'),'Location','SouthEast'), 
...
if(par.out.plotnetwork==1)
    ...
    gas_model_plotter_new(out.n0); title('Original Network Topology')
    if(par.out.plotpdf==1)
    set(gcf,'PaperPositionMode', 'manual','PaperUnits','points', ...
        'Paperposition',paperpos), set(gcf, 'PaperSize', papersize)
    ...
    gas_model_plotter_new(out.n);   title('Discretized Network Topology')
    if(par.out.plotpdf==1)
    set(gcf,'PaperPositionMode', 'manual','PaperUnits','points', ...
        'Paperposition',paperpos), set(gcf, 'PaperSize', papersize)
    eval(['print -dpdf ' mfolder '\13network.pdf']), end
